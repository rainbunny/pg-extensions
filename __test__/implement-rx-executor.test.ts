  ...
  it('runs create and returns id', (done) => {
    ...
    pool
      ...
      .subscribe((result) => {
        expect(
          (pool.query as unknown) as jest.Mock,
        ).toBeCalledWith('INSERT INTO app_user(username,displayName) VALUES($1,$2) RETURNING id', [
          'thinh',
          'Thinh Tran',
        ]);
        expect(result).toEqual(4);
  ...
  it('runs update with a specific id field', async (done) => {
    ...
    pool
      ...
      .subscribe(() => {
        expect(
          (pool.query as unknown) as jest.Mock,
        ).toBeCalledWith('UPDATE app_user SET username=$2,displayName=$3 WHERE userId = $1', [
          4,
          'thinh',
          'Thinh Tran',
        ]);
